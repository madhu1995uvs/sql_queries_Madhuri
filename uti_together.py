...
--cte_purpose: create variable of ua_positivity
,ua_le_cat as (select distinct
                              pt_fin as pt_fin_ua_le
                              ,result_dt_tm as RESULT_DT_TM_le
                              ,RESULT_MNEMONIC as urine_result_ua_le
                              ,result as result_ua_le
                              ...
                              from LAB_DATA_IMPORT_Master
                              where RESULT_MNEMONIC like '%Urine Leukocyte Esterase')      
               
,ua_nitrite_cat as (select distinct
                              pt_fin as pt_fin_ua_nit
                              ,result_dt_tm as RESULT_DT_TM_nit
                              ,RESULT_MNEMONIC as urine_result_ua_nit
                              ,result as result_ua_nit
                              ...
                              from LAB_DATA_IMPORT_Master
                              where RESULT_MNEMONIC like '%urine nitrite') 

,ua_wbc_cat as (select distinct
                              pt_fin as pt_fin_ua_wbc
                              ,result_dt_tm as RESULT_DT_TM_wbc
                              ,RESULT_MNEMONIC as urine_result_ua_wbc
                              ,result as result_ua_wbc
                              ...
                              from LAB_DATA_IMPORT_Master
                              where RESULT_MNEMONIC like '%urine wbc')     

, ua_final_cat as                  (select *
       , case when ua_wbc_cat.ua_wbc = 1 or ua_le_cat.ua_Le= 1 or ua_nitrite_cat.ua_nitrite= 1 then '1'
                                                                             else '0'
                              end ua_final
                              from ua_le_cat
                              full outer join ua_nitrite_cat on ua_le_cat.pt_fin_ua_le = ua_nitrite_cat.pt_fin_ua_nit 
                              full outer join ua_wbc_cat on ua_le_cat.pt_fin_ua_le = ua_wbc_cat.pt_fin_ua_wbc 
                              where RESULT_DT_TM_le BETWEEN @Start and @End)
...
, udip_wbc as (
       select pt_fin_udip as pt_fin_udip_wbc
       , result_dt_tm as RESULT_DT_TM_wbc
...
--cte_purpose: create temperature site, temperature result
, temp_first as 
               (
               select distinct
...
,orig_order_dt_tm, ucx_source,urine_culture_result
, positive_culture
...
left outer join ua_results on tat.PATIENT_FIN = ua_results.pt_fin_ua
...
, small_notes as (
             ...
             , case
                    ...
                    when (allergy_str like '%pre-med%')
                    then left(allergy_str,charindex('pre-med',allergy_str)-1)
                                                                                when (allergy_str like '%pre med%')
                    then left(allergy_str,charindex('pre med',allergy_str)-1)
...
, ordered_allergies as
                (
                select patient_fin_notes
                , allergy_substr
                , Medication_allergy
                , Row_Number() over (Partition By patient_fin_notes Order by Medication_allergy) as rn
                from allergy_notes
                )

, grouped_allergies as
                (
                select patient_fin_notes
                , max(case when rn = 1 then Medication_allergy end) allergy1
                --, max(case when rn = 1 then allergy_substr end) long_allergy1
                , max(case when rn = 2 then Medication_allergy end) allergy2
                --, max(case when rn = 2 then allergy_substr end) long_allergy2
                , max(case when rn = 3 then Medication_allergy end) allergy3
                --, max(case when rn = 3 then allergy_substr end) long_allergy3
                , max(case when rn = 4 then Medication_allergy end) allergy4
                --, max(case when rn = 4 then allergy_substr end) long_allergy4
                , max(case when rn = 5 then Medication_allergy end) allergy5
                --, max(case when rn = 5 then allergy_substr end) long_allergy5
                from ordered_allergies
                group by patient_fin_notes
                )

select * from grouped_allergies
where allergy1 is not null
and (allergy1 != allergy2 or allergy2 is null)
