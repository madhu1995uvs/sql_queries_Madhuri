...
#%%
"""CREATE LIST OF FINS TO USE IN THE SUBSEQUENT QUERY
...
#fin_tuple = tuple(uti.patient_fin.unique().tolist())
#fin_tuple = ('3000318558','3000318558','3000318558')
#fin_tstring = str(fin_tuple)
#fin_tstring = "'" + str(fin_tuple) + "'"
...
"""
#allergies = pd.read_sql(allergies_sql,conn,params=[fin_tuple])
#allergies = allergies.sort_values('patient_fin_notes')
#allg = allergies.drop_duplicates()


cycle = 0
for each_fin in uti1.patient_fin:
       fin_string = str(each_fin)
       if cycle == 0:
           allergies = pd.read_sql(allergies_sql,conn,params=[fin_string])
       else:
           allerg = pd.read_sql(allergies_sql,conn,params=[fin_string])
           allergies = allergies.append(allerg)
       cycle += 1
