...
,boluses as ( select *
  , two_bolus = '1'
   from bolus
  where (bolus_GIVEN_DT_TM_2 is not null))
...
,final as (
		...
		from tat
			...
			left outer join screen on  tat.patient_fin = screen_fin
				...
				and (osh is null or track_group like 'EDU%')
...
error_index = final_vitals[final_vitals['VitalSignValue'].str.contains('[a-z]')].index
final_vitals.drop(error_index, inplace=True)


# %%
final_vitals[final_vitals['VitalSignValue'].str.contains('D')]
