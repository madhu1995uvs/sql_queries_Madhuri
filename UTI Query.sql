...
, case
		when result_ua like '%>%'
		then 'result greater than number listed'
		when result_ua like '%<%'
		then 'result less than number listed'
		when result like '%-%'
		then 'lower number from range used'
		when result like '%+%'
		then 'result with +'
			
	end result_ua_modifiers
	*/
...
SET @End = '04/01/21';
	...
	-- 	add cte for udip le and nitrite and categorize udip and ua
...
with ua_results as
	(
		...
		or RESULT_MNEMONIC like '%urine wbc' or RESULT_MNEMONIC like '%urine nitrite') 
		and (result like '%[0-9]%' 
		and (result not like '%[a-z]%')
			or (result like '%large%' or result like '%moderate%' or result like '%small%' or result like '%ref range%'))
			)
...
, EDTAT as
	...
	from ED_TAT_MASTER tat
		...
		BILL on BILL.PATIENT_IDENTIFICATION = tat.PATIENT_FIN )
